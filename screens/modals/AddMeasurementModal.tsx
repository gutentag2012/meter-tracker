...
export default function AddMeasurementModal({
  navigation,
  route: {
    params: { meter, editMeasurement, onEndEditing },
  },
  ...
  useEffect(() => {
    ...
    repository.executeRaw<[Measurement]>(query).then((result) => {
      if (!result?.[0]) {
        setLastMeasurement(undefined)
      } else {
  ...
  const onSave = useCallback(async () => {
    ...
    if (!editMeasurement) {
      const newlyCreatedMeasurement = await repository.insertData(measurement)
      if (newlyCreatedMeasurement) {
        onEndEditing?.(newlyCreatedMeasurement)
      }
    } else {
      measurement.id = editMeasurement.id
      await repository.updateData(measurement)
      onEndEditing?.(measurement)
    }

    // switchTorch(false)
    setLoading(false)
    navigation.pop()
  }, [editMeasurement, errorState, navigation, onEndEditing, repository, selectedMeter])
