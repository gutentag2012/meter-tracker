...
import { Colors, Incubator, Text, type ToastProps, View } from 'react-native-ui-lib'
...
export const GlobalToast: FunctionComponent<Props> = ({ renderAttachment }) => {
  ...
  return (
    ...
    >
      ...
      >
        <ToastIcon size={24} color={toast?.colors?.text ?? Colors.onSecondaryContainer} />
        <Text
          style={{
        ...
        {toast?.isLoading && (
          <ActivityIndicator size={'small'} animating color={Colors.onSecondaryContainer} />
        )}
      </View>
    </Incubator.Toast>
  )
