...
export function getCellPositionFromIndex(index: number) {
  ...
  return {
    ...
    y: index
      ? Math.floor(index / CELL_AMOUNT_OF_COLUMNS) * (cellHeight + CELL_GAP)
      : 0,
  }
}
...
export function getCellContainerHeight(cellAmount: number) {
  const gapHeights =
    (Math.ceil(cellAmount / CELL_AMOUNT_OF_COLUMNS) - 1) * CELL_GAP
  return (
    Math.ceil(cellAmount / CELL_AMOUNT_OF_COLUMNS) * cellHeight + gapHeights
  )
}
...
export function getCellIndexFromPosition(x: number, y: number) {
  ...
  const column = Math.max(
    0,
    Math.floor((x + cellWidth / 3) / (cellWidth + CELL_GAP)),
  )
  const row = Math.max(
    0,
    Math.floor((y + cellHeight / 3) / (cellHeight + CELL_GAP)),
  )
